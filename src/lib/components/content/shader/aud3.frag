...
void main(){
    ...
    vec2 uv_2 = uv + vec2((sin(uTime / 2.0) * 0.25), 0.0);
    uv *= 14.0;
    uv_2 *= 14.0;
    vec3 c_1 = vec3(0.0392, 0.2471 + (uAudio / 2.0), 0.1176 + (uAudio / 2.0));
    vec3 c_2 = vec3(0.6 + (uAudio / 2.0), 0.4824, 0.102);

    float he = sin(uTime*0.43+4.0); he = (0.001+abs(he)) * ((he>=0.0)?1.0:-1.0);
    float ra = 0.1 + 0.5*(0.5+0.5*sin(uTime*1.7)) + max(0.0,he-0.7);
